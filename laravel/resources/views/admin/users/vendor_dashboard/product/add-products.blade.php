            ...
            <div class="col-sm-6">
              <div class="pro-add-left">
                ...
                <div class="pro-add-btn">
                    <form>
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-lg">Add a Product</button>
                    </form>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <!--  #######################women################### -->
                ...
                ?>
                 @foreach($sub_categories as $sc)
                  ...
                  <div class="col-sm-3 subcaterory">
                   ...
                   @foreach($sub_categories_types as $sct)
                    <ul class="subtype">
                      <li><a onclick="menre('{{$sct->name}}','{{$c->id}}','{{$sc->id}}','{{$sct->id}}');"> {{$sct->name}}</a></li>
                    </ul>
                    @endforeach
                  </div>
                  @endforeach
       ...
       <!--  #######################electronic################### -->
              ...
              @if($c->id == 677)
                <?php 
                $sub_categories = Manage::where('module_id', 2)->where('parent_id', 677)->where('is_deleted', 0)->orderBy('name')->get();
                ?>
                 @foreach($sub_categories as $sc)
                  <?php 
                    $sub_categories_types = Manage::where('module_id', 3)->where('parent_id', $sc->id)->where('is_deleted', 0)->orderBy('name')->get();
...
<div class="modal-content" id="beauty1" style="display:none;">
  <div class="bod-m">
    ...
    <div class="guid-btn">
        <form  action="/admin/tovendor/new-product">
